...
import { Components, ComputeItemKey, ContextProp, GroupContent, GroupItemContent, ItemContent, ListRootProps, SizeFunction } from './interfaces'
...
const Items = /*#__PURE__*/ React.memo(function VirtuosoItems({ showTopList = false }: { showTopList?: boolean }) {
  ...
  const ListComponent = useEmitterValue('ListComponent') as React.ElementType
  const ItemComponent = useEmitterValue('ItemComponent') as React.ElementType
...
const Header: React.FC = /*#__PURE__*/ React.memo(function VirtuosoHeader() {
  const Header = useEmitterValue('HeaderComponent') as React.ComponentType<ContextProp<any>> | undefined
...
const Footer: React.FC = /*#__PURE__*/ React.memo(function VirtuosoFooter() {
  const Footer = useEmitterValue('FooterComponent') as React.ComponentType<ContextProp<any>> | undefined
...
export function buildScroller({ useEmitter, useEmitterValue, usePublisher }: Hooks) {
  const Scroller: Components['Scroller'] = React.memo(function VirtuosoScroller({ children, style, context, ...props }) {
    ...
    const ScrollerComponent = useEmitterValue('ScrollerComponent') as React.ElementType
    const smoothScrollTargetReached = usePublisher('smoothScrollTargetReached')
    const scrollerRefCallback = useEmitterValue('scrollerRef') as (ref: HTMLElement | null | Window) => void
...
const Viewport: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const viewportRef = useSize(viewportSizeCallbackMemo, true, useEmitterValue('skipAnimationFrameInResizeObserver') as boolean)
...
const TopItemListContainer: React.FC<React.PropsWithChildren> = ({ children }) => {
  const TopItemList = (useEmitterValue('TopItemListComponent') as React.ElementType | undefined) || 'div'
  const headerHeight = useEmitterValue('headerHeight') as number
